...
export default function StoreProductsPage() {
  ...
  // Phase 8 — when productGroups is present, render one card per
  // styleGroup with a swatch strip. Falls back to the legacy flat grid
  // when the server hasn't shipped productGroups (older cached payload
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-10">
      ...
      {isLoading ? (
        ...
        // Phase 8 — grouped grid: one card per styleGroup, swatch strip,
        // hover swap shows the per-variant approved render (or the
        // variant photo if no render yet).
        <AnimatePresence mode="wait">
          <motion.div
            key={"groups:" + activeCategory + searchQuery}
          ...
          >
            {filteredGroups.map((g, i) => {
              ...
              return (
                <motion.div key={g.styleGroup} custom={i} variants={cardVariants}>
                  <VariantProductCard
                    ...
                    onSelect={slug => {
                      window.location.href = `?subPath=product/${encodeURIComponent(slug)}`;
                    }}
                  />
                </motion.div>
              );
